...
lambdaVals = [0.001,0.01,0.1,1,10,100];
...
#			[dataSave(contIters).costeVal void] = funCosteVal(thetaProc);
			...
			dataSave(contTam, contLambda, contIters).porcentajeEntren = porcentajeAciertos(ytrain, yPred);
			[a1, a2, z2, yPred] = red(ThetaProc1, ThetaProc2, X_val_unos);
			dataSave(contTam, contLambda, contIters).porcentajeTest = porcentajeAciertos(yval, yPred);
			printf("PorcentajeVal = %.4f\n", dataSave(contTam, contLambda, contIters).porcentajeTest);
...
save dataSavePruebaTotal dataSave;
